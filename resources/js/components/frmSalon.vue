                ...
                <div class="card">
                    <div class="card-header text-center text-white" style="background-color: #3399FF">
                            <div class="col-sm-12 bg-black d-flex justify-content-between align-items-center">
                                <h6 class="mb-0 col-md-11">SALON</h6>
                    ...
                    <div class="col-sm-6 col-lg-3">
                        <div class="card mb-4" style="--cui-card-cap-bg: #3399FF">
                            ...
                            <strong style='font-size: 0.8rem'> SALON </strong>
                            </div>
                            <div class="card-body row text-center">
                                ...
                                <div class="vr"></div>
                                <div class="col">
                                    <div class="fs-5 fw-semibold" style='font-size: 0.8rem'>{{salon_registro}}</div>
                    ...
                    <div class="form-group row">
                        <div class="col-md-8">
                            &nbsp;&nbsp;<div class="input-group" style='width:96%;margin-left: 3.3%;font-size: 0.8rem'>
                                ...
                                <!--<select class="form-select" v-model="tienda" @change="listarSalon(1, buscar, criterio)" style='font-size: 0.8rem'>
                                ...
                                &nbsp;&nbsp;&nbsp;
                                <input type="text" v-model="buscar" @keyup.enter="listarSalon(1, buscar, criterio)"  @keyup="BuscandoSalon()" class="form-control" placeholder="Texto a buscar" style='font-size: 0.8rem'>
                                &nbsp;&nbsp;&nbsp;
                                <button type="submit" @click="listarSalon(1, buscar, criterio)" class="btn btn-info text-white" style='font-size: 0.8rem'><i class="fa fa-search"></i> Buscar</button>
                            </div>
                        </div>
                    </div>
                    <br>
                    ...
                    <table class="table table-striped table-hover" style='width:96%;margin-left: 2.2%;font-size: 0.8rem'>
                        ...
                        <tbody>
                            <tr v-for="categoria in arraySalon" :key="categoria.id">
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                        ...
                        <form action="" method="post" enctype="multipart/form-data" class="form-horizontal">
                            ...
                            <!--<template v-if="usuario.id_tienda == 100">
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10"><select class="form-select" v-model="datos.id_tienda">
                            ...
                            <center>    
                                    ...
                                    <figure><img width="100" height="100" :src="imagenMiniatura" onerror="this.src='../img/producto/salon.png'" class="border border-1 border-dark" alt=""></figure>
                            </center> 
                    ...
                    <div class="modal-footer">
                        ...
                        <button :disabled="modificando" type="button" v-if="tipoAccion==2" class="btn btn-info text-white" @click="modificarSalon()">Modificar</button>
                    </div>
                </div>
                <!-- /.modal-content -->
            </div>
        ...
        methods : {
            ...
            listarSalon(page, buscar, criterio){
                ...
                if(me.usuario.id_tienda == 100){
                    var url='/salon?page=' + page + '&buscar=' + buscar + '&criterio=' + criterio + '&id_tienda=' + me.tienda;
                    axios.get(url).then(function(response){
                        me.arraySalon=response.data.data;
                ...
                {
                    var url='/salon?page=' + page + '&buscar=' + buscar + '&criterio=' + criterio + '&id_tienda=' + me.usuario.id_tienda;
                    axios.get(url).then(function(response){
                        me.arraySalon=response.data.data;
                        me.pagination={total:response.data.total, 
                            current_page:response.data.current_page,
            ...
            listarSalonBusquedaRapida(){
                let me=this;
                var url='/salon?page=' + 1 + '&buscar=' + me.buscar + '&criterio=' + me.criterio;
                axios.get(url).then(function(response){
                    me.arraySalon=response.data.data;
                    me.pagination={total:response.data.total, 
                        current_page:response.data.current_page,
            ...
            BuscandoSalon(){
                ...
                me.setTimeoutBuscador = setTimeout(me.listarSalonBusquedaRapida,350)
            },
            ...
            guardar(){
                if(this.datos.nombre == ''){
                    this.validacionError('El nombre del salon no puede estar vacio',3000); 
                }else{   
                    //if(this.is_busy == 0){
                        this.guardarSalon();
                        //this.is_busy=1;
            ...
            async guardarSalon(){
                try {
                    let me = this;
                    me.guardando=true;
                    me.datos.id_tienda=2;
                    const res = await axios.post('/salon/guardar',this.datos);
                    if(res.data.error==0){
                        //me.$toaster.error('Matricula ya existe...');
                        me.guardando=false;
                        me.validacionError("El salon ya existe", 2000);
            ...
            modificarSalon(){
                ...
                me.modificando=true;
                //if(me.is_busy==0){
                    axios.put('/salon/modificar',this.datos).then(function(response){

                        if(response.data.error==0){
                            me.modificando=false;
                            me.validacionError('El nombre del salon no puede estar vacio',3000); 
                    ...
                    case "categoria":
                        {
                            switch(accion){
                                case 'registrar':
                                    {
                                        ...
                                        this.imagenMiniatura = '';
                                        this.datos.imagenActual = '';
                                        this.tituloModal='Registro Salon'
                                        this.datos = {
                                            id : 0,
                                            nombre : '',
                                            descripcion : '',
                                ...
                                case 'modificar':
                                    {
                                        this.modal = 1;
                                        this.tituloModal = 'Modificar Salon';
                                        this.tipoAccion = 2;
            ...
            desactivarCategoria(id){
                ...
                swalWithBootstrapButtons.fire({
                    title: 'Esta seguro de Inhabilitar este Salon??',
                ...
                if (result.isConfirmed) {
                    let me = this;
                    axios.put('/salon/desactivar',{'id': id}).then(function (response) {
                        me.listarSalon(1,'', 'nombre');
                        swalWithBootstrapButtons.fire(
                        'Inhabilitado!',
                        'Este salon se ha Inhabilitado.',
                        'success'
                        )
                    }).catch(function (error) {
                        console.log(error);
                    ...
                    swalWithBootstrapButtons.fire(
                    'Cancelado',
                    'Este salon no ha tenido cambios :)',
                    'error'
                    )
                }
                })   
            ...
            activarCategoria(id){
                ...
                swalWithBootstrapButtons.fire({
                    title: 'Esta seguro de Habilitar este Salon??',
                    text: "Puede revertir esta decision!",
                    icon: 'warning',
                    showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    let me = this;
                    axios.put('/salon/activar',{'id': id}).then(function (response) {
                        me.listarSalon(1,'', 'nombre');
                        swalWithBootstrapButtons.fire(
                        'Habilitado!',
                        'Este salon se ha Habilitado.',
                        'success'
                        )
                    }).catch(function (error) {
                        console.log(error);
