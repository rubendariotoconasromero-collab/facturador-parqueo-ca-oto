		...
		template: `<div id="com-siat-puntos-venta">
			...
			<div class="mb-3">
				<div class="row g-1">
					...
					<div class="col-12 col-sm-2">
						<button type="button" class="btn btn-warning text-white" v-on:click="sync()">Sincronizar</button>
						<button type="button" class="btn btn-info text-white" v-on:click="filter()">Filtrar</button>
					</div>
					<div class="col-12 col-sm-2">
					</div>
				</div>
			...
			<div class="panel card border shadow mb-2" v-for="(item, index) in items" style="font-size:0.8rem;">
					...
					<div class="row">
						...
						<div class="col-12 col-sm-2">
							<template v-if="item.estado_recepcion == 'OBSERVADA'">
								<button type="button" class="btn btn-warning text-white mb-2 btn-block" v-on:click="validarCerrar(item, index)">Reenviar</button>
							</template>
							<template v-if="item.estado == 'OPEN'">
								<button type="button" class="btn btn-info text-white mb-2 btn-block" v-on:click="validarCerrar(item, index)">Cerrar Evento</button>
								<button type="button" class="btn btn-danger text-white mb-2 btn-block" v-on:click="confirmarAnular(item, index)">Anular Evento</button>
							</template>
							<template v-else>
								<!--<button type="button" class="btn btn-info text-white mb-2 btn-block" v-on:click="validarRecepcion(item, index)">Validar Recepcion</button>-->
							</template>
							<template v-if="item.codigo_recepcion_paquete">
								<button type="button" class="btn btn-info text-white mb-2 btn-block" v-on:click="detallesRecepcion(item, index)">Ver Detalles Recepcion</button>
							</template>
						</div>
					</div>
					<div class="row">
			...
			<div v-else>
				<i class="text-info">No se encontraron registros de eventos</i>
			</div>
			...
			<div ref="modalcerrar" class="modal fade">
				<div class="modal-dialog" style="max-width:800px; min-width:750px">
					<form ref="formcerrar" action="" method="" class="modal-content" novalidate>
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-danger text-white" data-dismiss="modal" data-bs-dismiss="modal" data-coreui-dismiss="modal">Cerrar</button>
							<button type="button" class="btn btn-info text-white" v-on:click="cerrarEvento()">Cerrar Evento</button>
						</div>
					</form>
				</div>
			</div>
		...
		methods: 
		{
			...
			async validarRecepcion(item, index)
			{
				try
				{
					...
					const res = await this.service.validarRecepcion(item.id);
					//console.log(res);
					this.$root.$processing.hide();
					this.filter();
				}
