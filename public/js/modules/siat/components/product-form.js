(function(ns)
{
	ns.ProductForm = {
		template: `<div class="form-invoices">
			<form ref="form" novalidate>
				...
				<div class="mb-3">
					...
					<select class="form-control form-select" required v-model="product.codigo_actividad">
				...
				<div class="mb-3 text-end">
					...
					<button type="button" class="btn btn-info text-white" v-on:click="save()">Guardar</button>
				</div>
			</form>
		</div>`,
		props: {
		...
		watch: 
		{
			'product.codigo_actividad':function codigo_actividad()
			{
				this.codigo_sin = '';
				this.filterProducts();
				console.log('diste');
		...
		methods: 
		{
			filterProducts()
			{
				//console.log('clave'+this.product.codigo_actividad);
				this.codigo_actividad=this.product.codigo_actividad;
				this.productos = [];
