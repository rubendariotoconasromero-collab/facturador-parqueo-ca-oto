(function(ns)
{
	class ServiceInvoices extends SBFramework.Classes.Api
	{
		...
		async obtenerFacturas(page, fecha_inicio, fecha_fin, id_usuario)
		{
			const res = await this.Get('/invoices/siat/v2/invoices?page=' + page+'&fecha_inicio='+fecha_inicio+'&fecha_fin='+fecha_fin+'&id_usuario='+id_usuario);
		...
		async buscar(keyword, branch, fecha_inicio, fecha_fin, id_usuario)
		{
			const res = await this.Get(`/invoices/search?keyword=${keyword}&sucursal=${branch}&fecha_inicio=`+fecha_inicio+`&fecha_fin=`+fecha_fin+`&id_usuario=`+id_usuario);
			
			return res;
		}
		async cargarProductos(){
			const res = await this.Get(`/invoices/siat/v2/get_productos`);
			return res;
		}
	}
